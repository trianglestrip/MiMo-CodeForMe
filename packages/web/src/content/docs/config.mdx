...
- `max_context` - Compact earlier than the model's own window. Either one value for every model (`"300K"`) or a map keyed by `"<providerID>/<modelID>"` with wildcards. Values may be a token count (`300000`), a shorthand string (`"300K"`, `"1M"`) or a percentage of the model window (`"50%"`). The value is always clamped to the model's real window, so it can only lower the compaction trigger, never raise it. `0` means no budget for that model.

The compaction trigger is the budget minus the reserves, so `"max_context": "300K"` on a
model with a 20K output reserve and a 20K compaction buffer compacts at 260K. Run
`mimocode models <provider>` to print each model's window and where it compacts, or use
the `/context-limit` command in the TUI to set the budget for the current model from
preset tiers.
